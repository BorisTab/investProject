...
class MeanReversionAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.rsiPeriod = 14
        self.rsiSellThreshold = 80
        self.rsiBuyThreshold = 20
        self.rsi = self.RSI(self.symbols[0], self.rsiPeriod, MovingAverageType.Simple, Resolution.Hour)

    def OnData(self, data):
        for symbol in self.symbols:
            if not self.Portfolio[symbol].Invested:
                if self.IsMeanReversionBuySignal(symbol) and self.rsi.Current.Value < self.rsiBuyThreshold:
                    self.SetHoldings(symbol, 1)
                    self.Debug(f"Bought {symbol} at {self.Securities[symbol].Price}")
            else:
                if self.IsMeanReversionSellSignal(symbol) and self.rsi.Current.Value > self.rsiSellThreshold:
