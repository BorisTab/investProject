...
class MovingAverageCrossoverAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.rsiPeriod = 14
        self.rsiBuyThreshold = 30
        self.rsiSellThreshold = 70
        self.rsi = self.RSI(btcusd, self.rsiPeriod, MovingAverageType.Simple, Resolution.Minute)
    ...
    def OnData(self, data):
        ...
        if self.previous_fast_ma and self.previous_slow_ma:
            if self.previous_fast_ma < self.previous_slow_ma and current_fast_ma > current_slow_ma and self.rsi.Current.Value < self.rsiBuyThreshold:
                self.buying = True
            elif self.previous_fast_ma > self.previous_slow_ma and current_fast_ma < current_slow_ma and self.rsi.Current.Value > self.rsiSellThreshold:
                self.selling = True
